...
;
;
(defun QT:DRAWENVELOPE2 (/ ANGLIST AREA AREA0 BASELIST C C1 ENT ENT0 ENTSET ENTSET2 ENTSET3 LM:outline LM:ssboundingbox NODE OBJ P PA PB PC PD PLIST0 PLIST1 VLIST)
  ...
  (progn
   ...
   (if (setq VLIST (QT:GETENVELOPE ENT))
    (progn
     ...
     (setq C 0)
     (foreach P VLIST
      (setq PLIST0 (append PLIST0 (list (list (+ (nth 0 (nth C BASELIST)) (* (nth 0 P) (cos (nth C ANGLIST))) (* -1.0 (nth 1 P) (sin (nth C ANGLIST))))
     ...
     (while (< C (length ANGLIST))
      (setq PLIST1 nil)
      (foreach P VLIST
       (setq PLIST1 (append PLIST1 (list (list (+ (nth 0 (nth C BASELIST)) (* (nth 0 P) (cos (nth C ANGLIST))) (* -1.0 (nth 1 P) (sin (nth C ANGLIST))))
                                               (+ (nth 1 (nth C BASELIST)) (* (nth 0 P) (sin (nth C ANGLIST))) (* (nth 1 P) (cos (nth C ANGLIST))))
                                         )
                                   )
                    )
      ...
      (setq C1 0)
      (while (< C1 (length PLIST0))
       (setq PA (nth C1 PLIST0))
       (setq PB (nth (if (= (1+ C1) (length PLIST0)) 0 (1+ C1)) PLIST0))
       (setq PC (nth C1 PLIST1))
       (setq PD (nth (if (= (1+ C1) (length PLIST1)) 0 (1+ C1)) PLIST1))
...
;       (command "._PLINE" "NONE" PA "NONE" PB "NONE" PD "NONE" PC "CLOSE")
